...
@section('content')
@php
    $initialRole = old('role', $currentRole);
    $checkedPermissions = old('permissions', $selectedPermissions);
@endphp

<h1 class="mb-4 text-xl font-semibold">Edit Pengguna</h1>
...
<div class="grid gap-6 xl:grid-cols-[minmax(0,38rem)_minmax(0,1fr)]">
  <div class="rounded-lg border bg-white p-4">
    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded border border-gray-300 px-3 py-2 text-sm">
        @error('name')<div class="mt-1 text-xs text-red-600">{{ $message }}</div>@enderror
      </div>

      <div>
        <label class="mb-1 block text-sm font-medium text-gray-700">Email</label>
        <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded border border-gray-300 px-3 py-2 text-sm">
      ...
      <div>
        ...
        <div class="mt-1 text-xs text-gray-500">Kamu bisa pertahankan hak akses yang sudah ada, atau isi ulang mengikuti default role.</div>
      ...
      <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
        <div class="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <p class="text-xs text-slate-500">Centang menu yang boleh diakses pengguna ini.</p>
          </div>
          <button
            type="button"
  ...
  <div class="rounded-lg border bg-white p-4">
    <h2 class="text-sm font-semibold text-slate-900">Ringkasan Pengguna</h2>
    <dl class="mt-3 space-y-2 text-sm">
      <div class="flex items-center justify-between gap-3">
        <dt class="text-slate-500">Role aktif</dt>
        <dd class="font-medium text-slate-900">{{ strtoupper($currentRole ?? '-') }}</dd>
      </div>
      <div class="flex items-center justify-between gap-3">
        <dt class="text-slate-500">Hak akses saat ini</dt>
        <dd class="font-medium text-slate-900">{{ count($selectedPermissions) }} menu</dd>
      </div>
    </dl>
    <p class="mt-4 text-sm text-slate-600">Kalau role diubah, kamu bisa klik <span class="font-medium">Gunakan Default Role</span> untuk mengisi ulang checklist dengan pola bawaan jabatan tersebut.</p>
  </div>
</div>
