...
@section('content')
<h1 class="text-xl font-semibold mb-4">Tambah Laporan Kerusakan/Pemusnahan</h1>

<div class="bg-white border rounded-lg p-4 max-w-2xl">
  <form method="POST" action="{{ route('admin.asset_incidents.store') }}" class="space-y-4">
    @csrf
    ...
    <div>
      ...
      <select name="type" class="w-full rounded border border-gray-300 px-3 py-2 text-sm">
        <option value="DAMAGE" @selected(old('type') === 'DAMAGE')>DAMAGE</option>
        <option value="DISPOSAL" @selected(old('type') === 'DISPOSAL')>DISPOSAL</option>
      </select>
    </div>
    ...
    <div>
      ...
      <textarea name="description" class="w-full rounded border border-gray-300 px-3 py-2 text-sm" rows="3">{{ old('description') }}</textarea>
    </div>

    <div>
      <label class="block text-sm font-medium text-gray-700 mb-1">Biaya</label>
      <input type="number" name="cost" value="{{ old('cost', 0) }}" min="0" step="1000"
             class="w-full rounded border border-gray-300 px-3 py-2 text-sm">
    </div>
